...
type
 ...
 //   property ID;
  ...
  public
    property DefaultInterface: ITaskbarList3 read FTaskBarList3;
...
begin
  ...
  self.FTaskBarList3.SetProgressValue(Application.MainFormHandle,p,m);
end;
...
begin
  inherited;
  FButtonsAdded:=False;
  FTaskBarList3:=GetTaskbarList3Interface;
  FThumbBarButtons:=TThumbBarButtons.Create(self);

  if (AOwner is TWinControl) and (not (csDesigning in Self.ComponentState)) then
  begin
...
begin
  ...
  begin
    ...
    FTaskBarList3.ThumbBarAddButtons(Application.MainFormHandle, Length(Buttons), @Buttons[0]);
    FButtonsAdded:=True;
...
begin
  ...
  begin
    FOwner.FTaskBarList3.ThumbBarUpdateButtons(Application.MainFormHandle, Length(Buttons), @Buttons[0]);

    if Assigned(FImages) then
      TTaskBarList(Owner).FTaskBarList3.ThumbBarSetImageList(Application.MainFormHandle, FImages.Handle)
    else
      TTaskBarList(Owner).FTaskBarList3.ThumbBarSetImageList(Application.MainFormHandle, 0);
  end;
end;
