...
type
  ...
  TfrxCustomUIBDatabase = class(TfrxCustomDatabase)
  private
  ...
  private
    FDatabase: TfrxCustomUIBDatabase;
    FTransaction: TUIBTransaction;
  protected
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    procedure SetDatabase(const Value: TfrxCustomUIBDatabase);
  ...
  published
    property Database: TfrxCustomUIBDatabase read FDatabase write SetDatabase;
...
class function TfrxCustomUIBDatabase.GetDescription: String;
begin
  Result := frxResources.Get('obUIBDB');
end;

function TfrxCustomUIBDatabase.GetConnected: Boolean;
begin
  Result := FDatabase.Connected;
end;

function TfrxCustomUIBDatabase.GetDatabaseName: String;
begin
  Result := FDatabase.DatabaseName;
end;

function TfrxCustomUIBDatabase.GetLoginPrompt: Boolean;
begin
  Result := false;
end;

function TfrxCustomUIBDatabase.GetParams: TStrings;
begin
  Result := FDatabase.Params;
end;

function TfrxCustomUIBDatabase.GetSQLDialect: Integer;
begin
  Result := FDatabase.SQLDialect;
end;

procedure TfrxCustomUIBDatabase.SetConnected(Value: Boolean);
begin
  FDatabase.Connected := Value;
end;

procedure TfrxCustomUIBDatabase.SetDatabaseName(const Value: String);
begin
  FDatabase.DatabaseName := Value;
end;

procedure TfrxCustomUIBDatabase.SetLoginPrompt(Value: Boolean);
begin
  // not supported...
end;

procedure TfrxCustomUIBDatabase.SetParams(Value: TStrings);
begin
  FDatabase.Params := Value;
end;

procedure TfrxCustomUIBDatabase.SetSQLDialect(const Value: Integer);
begin
  FDatabase.SQLDialect := Value;
end;

procedure TfrxCustomUIBDatabase.SetLogin(const Login, Password: String);
begin
...
begin
  ...
  for i := 0 to l.Count - 1 do
    if TObject(l[i]) is TfrxCustomUIBDatabase then
    begin
      SetDatabase(TfrxCustomUIBDatabase(l[i]));
      Break;
    end;
end;
...
procedure TfrxUIBTransaction.SetDatabase(const Value: TfrxCustomUIBDatabase);
begin
  FDatabase := Value;
  if Value <> nil then
...
procedure TfrxUIBQuery.SetDatabase(const Value: TfrxCustomUIBDatabase);
begin
  FDatabase := Value;
...
{$IFDEF UIB_IMAGES}
  frxObjects.RegisterObject(TfrxCustomUIBDatabase, UIBDatabaseBmp, 'UIB');
  frxObjects.RegisterObject(TfrxUIBTransaction, UIBTransactionBmp, 'UIB');
  frxObjects.RegisterObject(TfrxUIBQuery, UIBQueryBmp, 'UIB');
{$ELSE}
  frxObjects.RegisterObject1(TfrxCustomUIBDatabase, nil, '', 'UIB', 0, 37);
