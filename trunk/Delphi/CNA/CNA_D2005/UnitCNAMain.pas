...
begin
  ...
  try
    if (Regi.OpenKey('Software\Borland\BDS\3.0\Environment Variables\',false)) then
...
begin
  ...
  begin
    ...
    begin
      if FToolForm.Controls[i].ClassType = TPanel then
        FPanel:=TPanel(FToolForm.Controls[i]);
  ...
  if (FPanel<>nil) and (FCatButtons<>nil) then
  begin
    xPos:=0;
    for i:=0 to FPanel.ControlCount-1 do
    begin
      if TControl(FPanel.Controls[i]).Left+TControl(FPanel.Controls[i]).Width>xPos then
          xPos:=TControl(FPanel.Controls[i]).Left+TControl(FPanel.Controls[i]).Width;
    end;
