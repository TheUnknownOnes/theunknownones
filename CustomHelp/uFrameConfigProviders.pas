...
type
  ...
  private
    ...
    function IsURLAllowed(AFilter: TConfigProvidersFilter; AURL: string): Boolean;
...
begin
  if (ListView1.ItemFocused <> FInsertItem) and Assigned(ListView1.ItemFocused) then
...
function TFrameConfigProviders.IsURLAllowed(AFilter: TConfigProvidersFilter;
  AURL: string): Boolean;
begin
  Result := False;
...
procedure TFrameConfigProviders.ListView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if (ListView1.ItemFocused <> FInsertItem) and Assigned(ListView1.ItemFocused) and
    (Key = VK_DELETE) and (MessageDlg('Are you sure to delete ' +
    ListView1.ItemFocused.Caption + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
...
function TFrameConfigProviders.TrimOptionFromString(AString: string):
  ...
  case AFilter of
    ...
    begin
      ListView1.Columns[2].Caption :=
        Format(ListView1.Columns[2].Caption, ['Path/Shell command']);
      LabelURLPath.Caption := Format(LabelURLPath.Caption, ['Path/Command']);
      edURL.Hint :=
        '- Path to a windows *.hlp file (be sure to have winhlp32.exe installed)' +
        #13#10 + '- Path to a windows htmlHelp file (*.chm)' + #13#10 +
