...
begin
  ...
  begin
    s:=Group.PropertiesAvailable.ValueFromIndex[PropAvailI];
    if AnsiStartsStr('[',s) then
    begin
      tmpStr.CommaText:=copy(s,2,length(s)-2);
      for i:=0 to tmpStr.Count-1 do
        tmpStr.Objects[i]:=TObject(StrToInt(tmpStr.ValueFromIndex[i]));
    end;
    TypeK:=TTypeKind(Group.PropertiesAvailable.Objects[PropAvailI]);
    case TypeK of
      tkEnumeration:
        begin
          i:=tmpStr.IndexOfObject(TObject(StrToIntDef(Group.PropertiesSet.ValueFromIndex[PropSetI],0)));
          if i<0 then
            i:=0;  
      ...
      tkSet:
        begin
          Value:=StrToIntDef(Group.PropertiesSet.ValueFromIndex[PropSetI],0);
          for i:=0 to tmpStr.Count-1 do
          begin
      ...
      Result:=Group.PropertiesSet.ValueFromIndex[PropSetI];
...
begin
  ...
  try
    ...
    MyForm.GetNewValue(Group.PropertiesSet.Values[Lv.Selected.Caption],
                       ...
                       Group.PropertiesAvailable.ValueFromIndex[Group.PropertiesAvailable.IndexOfName(Lv.Selected.Caption)],
                       false,
                       NewVal,
                       ShowInputBox);
