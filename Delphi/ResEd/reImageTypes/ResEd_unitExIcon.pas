(*======================================================================*
 ...
 |     max of 256.                                                      |
 |                                                                      |
 | based on unitExIcon.pas Copyright © Colin Wilson 2002                |
 | modified by Marco Warm to support PNG Compressed icons               |
 *======================================================================*)

unit ResEd_unitExIcon;
...
TExIconImage = class (TSharedImage)
  ...
  procedure CheckForPNGCompression;
private
...
public
  ...
  property PNGCompressed : Boolean read FPNGCompressed;
end;
...
function SetPixelFormatFromPNG(APNG: TPNGObject): TPixelFormat;
begin
  case APNG.Header.BitDepth of
      1  : result := pf1bit;
      4  : result := pf4bit;
      8  : result := pf8bit;
...
begin
  with fImages [fCurrentImage] do
    ...
    begin
      if FImages[FCurrentImage].PNGCompressed then
      begin
        fImages [fCurrentImage].FPNGImage.Draw(ACanvas, Rect);
      end
      else
      begin
        info := GetBitmapInfo;
...
begin
  ...
  begin
    ...
    FImages[i].CheckForPNGCompression;

    if not FImages[i].PNGCompressed then
    begin
    ...
    begin
      fImages [i].FPNGImage.Header.BitDepth:=dirEntry [i].wBitCount;
      fImages [i].FPixelFormat:=SetPixelFormatFromPNG(fImages [i].FPNGImage);
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        fImages [i].FPNGImage.Header.BitDepth:=resDir.wBitCount;
        fImages [i].FPixelFormat:=SetPixelFormatFromPNG(fImages [i].FPNGImage);
      end;
    finally
      strm1.Free
...
begin
  ...
  try
    ...
    begin
      ...
      if image.PNGCompressed then
      begin
        dirEntry.bWidth := 0;
        dirEntry.bHeight := 0;
        dirEntry.bColorCount := 0;

        dirEntry.wBitCount := image.FPNGImage.Header.BitDepth;
      end
      else
      begin
        dirEntry.bWidth := image.Width;
...
begin
  ...
  FPNGCompressed:=Header=pngHeader;

  FMemoryImage.Seek(0, soBeginning);

  if FPNGCompressed then
  Begin
    if Assigned(FPNGImage) then
      FPNGImage.Free;

    FPNGImage:=TPNGObject.Create;
    FPNGImage.LoadFromStream(FMemoryImage);

    FWidth:=FPNGImage.Width;
    FHeight:=FPNGImage.Height;
  End;
end;
