...
const C16KTable : array[0..106] of AnsiString = ('212222', '222122', '222221', '121223', '121322', '131222', '122213',
...
const C16KStartStop : array[0..7] of AnsiString = ('3211', '2221', '2122', '1411', '1132', '1231', '1114', '3112');
...
procedure c16k_set_a(source : AnsiChar; var values : TArrayOfInteger; var bar_chars : Integer);
var
  _source : Byte absolute source;
begin
  _source := _source and 127; { limit the range to 0-127 }
  if (_source < 32) then
    Inc(_source, 64)
  else
    Dec(_source, 32);
...
procedure c16k_set_b(source : AnsiChar; var values : TArrayOfInteger; var bar_chars : Integer);
var
  _source : Byte absolute source;
begin
  _source := _source and 127; { limit the range to 0-127 }
  values[bar_chars] := _source - 32;
  Inc(bar_chars);
end;

procedure c16k_set_c(source_a : AnsiChar; source_b : AnsiChar; var values : TArrayOfInteger; var bar_chars : Integer);
var
  weight : Integer;
begin
...
begin
  ...
  SetLength(_set, input_length);
  SetLength(fset, input_length);
  FillChar(_set[1], input_length, ' ');
  FillChar(fset[1], input_length, ' ');
  ...
  if ((gs1 <> 0) and (source[indexchaine] = '[')) then mode := ABORC; { FNC1 }
  ...
  begin
    ...
    if (i = 1) then
    begin
      if ((_set[i] = 'B') and (_set[2] = 'C')) then
        glyph_count := glyph_count - 1.0;

      if ((_set[i] = 'B') and (_set[2] = 'B')) then
      begin
        if (_set[3] = 'C') then
          glyph_count := glyph_count - 1.0;
      end;
    ...
    if ((_set[i] = 'C') and (not ((gs1 <> 0) and (source[i] = '[')))) then
      glyph_count := glyph_count + 0.5
    else
      glyph_count := glyph_count + 1.0;
  end;

  if ((gs1 <> 0) and (_set[1] <> 'A')) then
  ...
  begin
    ...
    begin
      if ((_set[1] = 'B') and (_set[2] = 'C')) then m := 6;
    end;
  ...
  begin
    ...
    begin
      if ((_set[1] = 'B') and (_set[2] = 'C')) then m := 5;
      if (((_set[1] = 'B') and (_set[2] = 'B')) and (_set[3] = 'C')) then m := 6;
    end;
    values[bar_characters] := (7 * (rows_needed - 2)) + m; { see 4.3.4.2 }
    Inc(bar_characters);
  end;
  ...
  repeat
    ...
    begin { Latch different code set }
      case _set[read] of
        ...
        'C':
        begin
          if (not ((read = 2) and (_set[1] = 'B'))) then
          begin { Not Mode C/Shift B }
            if (not ((read = 3) and ((_set[1] = 'B') and (_set[2] = 'B')))) then
            begin
    ...
    if (not ((gs1 <> 0) and (source[read] = '['))) then
    begin
  ...
  begin
    ...
    writer := 0;
    flip_flop := 1;
    for mx_reader := 1 to strlen(width_pattern) do
    begin
