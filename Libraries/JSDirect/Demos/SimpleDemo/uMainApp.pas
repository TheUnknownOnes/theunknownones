...
begin
  ...
  doc:= TjsHTMLDocument.Create(Self, 'document');
  xt:=((doc.createTextNode('Das ist ein lustiger Text') as IjsElement).ConvertTo(TjsHTMLElement) as IHTMLElement);
  body:=doc.body as IHTMLBodyElement;
  body.appendChild(xt);
  xt.nodeValue:='dsafdsaf';
  xt:=(doc.createElement('br') as IjsElement).ConvertTo(TjsHTMLElement) as IHTMLELement;
  body.appendChild(xt);
end;
